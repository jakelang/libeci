...
mod tests {
    ...
    fn test_no_startfn_good() {
        ...
        let mut checker = EcicChecker::default(&wasm);
        assert_eq!(checker.checks.get_check_status("no-startfn"), CheckStatus::Unknown);
        checker.fire();
        assert_eq!(checker.checks.get_check_status("no-startfn"), CheckStatus::Good);
    }
    ...
    fn test_has_startfn() {
        let wasm: Vec<u8> = vec!(0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x04, 0x01, 0x60,
            0x00, 0x00, 0x03, 0x02, 0x01, 0x00, 0x07, 0x08, 0x01, 0x04, 0x6d, 0x61,
            0x69, 0x6e, 0x00, 0x00, 0x08, 0x01, 0x00, 0x0a, 0x04, 0x01, 0x02, 0x00, 0x0b);
        let mut checker = EcicChecker::default(&wasm);
        assert_eq!(checker.checks.get_check_status("no-startfn"), CheckStatus::Unknown);
        checker.fire();
        assert_eq!(checker.checks.get_check_status("no-startfn"), CheckStatus::Malformed);
    }

    #[test]
    fn test_correct_import_namespace() {
        let wasm: Vec<u8> = vec!(0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x09, 0x02, 0x60,
            0x02, 0x7f, 0x7f, 0x00, 0x60, 0x00, 0x00, 0x02, 0x17, 0x01, 0x08, 0x65,
            0x74, 0x68, 0x65, 0x72, 0x65, 0x75, 0x6d, 0x0a, 0x67, 0x65, 0x74, 0x42,
            0x61, 0x6c, 0x61, 0x6e, 0x63, 0x65, 0x00, 0x00, 0x03, 0x02, 0x01, 0x01,
            0x07, 0x08, 0x01, 0x04, 0x6d, 0x61, 0x69, 0x6e, 0x00, 0x01, 0x0a, 0x04,
            0x01, 0x02, 0x00, 0x0b);
        let mut checker = EcicChecker::default(&wasm);
        assert_eq!(checker.checks.get_check_status("eei-imports"), CheckStatus::Unknown);
        checker.fire();
    ...
    fn test_incorrect_import_namespace() {
        let wasm: Vec<u8> = vec!(0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x09, 0x02, 0x60,
            0x02, 0x7f, 0x7f, 0x00, 0x60, 0x00, 0x00, 0x02, 0x12, 0x01, 0x03, 0x65,
            0x6e, 0x76, 0x0a, 0x67, 0x65, 0x74, 0x42, 0x61, 0x6c, 0x61, 0x6e, 0x63,
            0x65, 0x00, 0x00, 0x03, 0x02, 0x01, 0x01, 0x07, 0x08, 0x01, 0x04, 0x6d,
            0x61, 0x69, 0x6e, 0x00, 0x01, 0x0a, 0x04, 0x01, 0x02, 0x00, 0x0b);
        let mut checker = EcicChecker::default(&wasm);
        assert_eq!(checker.checks.get_check_status("eei-imports"), CheckStatus::Unknown);
        checker.fire();
        assert_eq!(checker.checks.get_check_status("eei-imports"), CheckStatus::Malformed);
    }
}
